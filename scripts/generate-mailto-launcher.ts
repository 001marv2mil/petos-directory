/**
 * Generates outreach/send-pitches.html — a local HTML file with one-click
 * buttons that open your default email client pre-filled with subject + body
 * for each pitch. Review, click Send.
 *
 * This is the FOCUSED version: 8 high-quality targets only. No tip-line spam,
 * no wrong-beat misses. Each recipient is someone whose actual job is to
...
// ─── The 8 high-quality pitches ────────────────────────────────────────────
const PITCHES: Pitch[] = [
  {
    id: 1,
    outletName: 'Pet Age — Glenn Polyn (Editor-in-Chief)',
    reason: 'Editor of a pet industry trade. Lives for this data.',
    contact: 'gpolyn@petage.com',
    subject: `New data: 8 major US cities have only 1 emergency vet — America's vet shortage mapped`,
    body: petPressBody('Hi Glenn,'),
    category: 'pet-press',
  },
  {
    id: 2,
    outletName: "Today's Veterinary Business — Sarah Rumple Mahan (Editor)",
    reason: 'Vet industry trade editor. Vet workforce shortage is her beat.',
    contact: 'smahan@navc.com',
    subject: `New data: 8 major US cities have only 1 emergency vet — America's vet shortage mapped`,
    body: petPressBody('Hi Sarah,'),
    category: 'pet-press',
  },
  {
    id: 3,
    outletName: 'Kinship (editorial)',
    reason: 'Condé Nast pet brand, huge audience, actively publishes data stories.',
    contact: 'editorial@kinship.com',
    subject: `New data: 8 major US cities have only 1 emergency vet — America's vet shortage mapped`,
    body: petPressBody('Hi Kinship team,'),
    category: 'pet-press',
  },
  {
    id: 4,
    outletName: 'DVM360 (news tips)',
    reason: 'Vet industry news tip line — they cover exactly this kind of shortage story.',
    contact: 'news@dvm360.com',
    subject: `New data: 8 major US cities have only 1 emergency vet — America's vet shortage mapped`,
    body: petPressBody('Hi DVM360 team,'),
    category: 'pet-press',
  },
  {
    id: 5,
    outletName: 'MinnPost — Andy Steiner (lifestyle/health reporter)',
    reason: 'Lifestyle/health beat in Minnesota. St. Paul vet desert is her lane.',
    contact: 'asteiner@minnpost.com',
    subject: `St. Paul data: only 1 emergency vet for the entire city (PetOS research)`,
    body: cityBody('St. Paul', 'Hi Andy,'),
    category: 'desert-city',
  },
  {
    id: 6,
    outletName: 'Kansas City Star — Greg Farmer (Executive Editor)',
    reason: 'Will route to the right reporter if he doesn\'t cover it himself. Overland Park is in KC metro.',
    contact: 'gfarmer@kcstar.com',
    subject: `Overland Park data: only 1 emergency vet for the entire city (PetOS research)`,
    body: cityBody('Overland Park', 'Hi Greg,'),
    category: 'desert-city',
  },
  {
    id: 7,
    outletName: 'Signal Cleveland — Frank W. Lewis (Editors Bureau)',
    reason: 'Local Cleveland editor. Cares about Cleveland-specific stories.',
    contact: 'frank@signalcleveland.org',
    subject: `Cleveland data: only 1 emergency vet for the entire city (PetOS research)`,
    body: cityBody('Cleveland', 'Hi Frank,'),
    category: 'desert-city',
  },
  {
    id: 8,
    outletName: 'Missoula Current — Martin Kidston (Editor)',
    reason: 'Independent local editor. Will take a Missoula-specific data angle.',
    contact: 'editor@missoulacurrent.com',
    subject: `Missoula data: only 1 emergency vet for the entire city (PetOS research)`,
    body: cityBody('Missoula', 'Hi Martin,'),
    category: 'desert-city',
  },
]
...
function main() {
  const cards: string[] = []
  cards.push(`<h2>🎯 The 8 high-quality pitches</h2>`)
  cards.push(`<p class="section-note">
    Each of these 8 people has a job description that maps directly to your story.
    Expected response rate: ~15–25% (1–2 replies).
  </p>`)
  for (const p of PITCHES) cards.push(pitchCard(p))

  const html = `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8"/>
<title>PetOS Pitch Launcher — 8 targeted pitches</title>
<style>
  * { box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 780px; margin: 0 auto; padding: 32px 16px; background: #fafafa; color: #111; line-height: 1.5; }
...
<p class="lead">
  8 high-quality recipients. Each with a job description that maps to your emergency-vet shortage story.
  One click per button opens Outlook (or whichever app you have set as default mail client) with the subject and body pre-filled.
  Review, then click Send in the email.
</p>

<div class="schedule">
  <h3>📅 Your 4-day schedule (2/day)</h3>
  <ol>
    <li><strong>Day 1 (today):</strong> #1 Pet Age (Glenn) + #2 Today's Vet Business (Sarah)</li>
    <li><strong>Day 2:</strong> #3 Kinship + #4 DVM360</li>
    <li><strong>Day 3:</strong> #5 MinnPost (Andy) + #6 KC Star (Greg)</li>
    <li><strong>Day 4:</strong> #7 Signal Cleveland (Frank) + #8 Missoula Current (Martin)</li>
  </ol>
</div>

<div class="tracker-note">
  <strong>Before you click the first button:</strong> Install <a href="https://mailtrack.io" target="_blank">Mailtrack.io</a>
  (free Gmail extension) or enable Outlook's read receipts so you see when each email is opened.
  After each send, log it in <code>outreach/backlink-tracker.csv</code>.
</div>

${cards.join('\n')}

<p class="lead" style="margin-top: 40px; text-align: center; color: #9ca3af;">
  Generated ${new Date().toISOString().slice(0, 10)} by scripts/generate-mailto-launcher.ts
</p>
</body>
</html>`
  ...
  console.log(`  ${PITCHES.length} high-quality pitches ready`)
  console.log(`  → Double-click the file to open in your browser, then click each button`)
}
